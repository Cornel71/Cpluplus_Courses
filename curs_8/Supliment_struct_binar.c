...
void afisareContinutFisierBinar(char numeFisier[], PERSOANA a[], int *n)
{
    ...
    fseek(fp,0,2);  // fseek(fp,0,SEEK_END);/* ne pozitionam la finalul fisierului */
    *n = ftell(fp)/sizeof(PERSOANA); /* pentru a determina cate elemete de tip structura sunt in fisier */
    ...
    for(i=0; i<*n; i++)
    {
        fread(&a[i],sizeof(PERSOANA),1,fp); /* citim din fisier fiecare structura, o punem intr-un vector*/
...
void citireContinutFisierBinar(char numeFisier[], PERSOANA a[], int *n)
{
    ...
    fseek(fp,0,2); /* ne pozitionam la sfarsitul fisierului */
